...
mapa = list()
for i in range(32): mapa.append(dict(zip(alfabet, ''.join((alfabet[i:],alfabet[:i])))))
...
def caesar(s,k):
    s = s.lower()
    wynik = ''.join([mapa[k][l] for l in s])
    return wynik
...
def parycezarskie(slownik=importlisty()):
    ...
    for i in range(max(dl.keys()),min(dl.keys())-1,-1):
        ...
        while zbior:
            x = zbior.pop()
            temp = x
            while temp[0] != 'a': temp = caesar(temp,1)
            if temp not in slownik: slownik[temp] = [x]
            else: slownik[temp].append(x)
