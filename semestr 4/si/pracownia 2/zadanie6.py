'''
Heurystyka optymistyczna gwarantuje, że znalezione rozwiązanie będzie optymalne. Niestety użycie takiej heurystyki oznacza
również, że A* musi zbadać wszystkie 'równowartościowe' ścieżki żeby znależć tą optymalną. Poprzez złagodzenie kryterium
dopuszczalności możemy przyśpieszyć wyszukiwanie kosztem optymalności. Po przemnożeniu heurystyki przez małą stałą algorytm
będzie bardziej preferował węzły, które wydają się być bliższe celowi.
'''
...
def priority(state):
    return 1.037 * max(distances[position] for position in state.positions) + len(state.movs)
