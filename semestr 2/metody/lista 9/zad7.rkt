...
(define (reserved? s)
  (or (member s '(let letrec if lambda true false and or quit)))) ; quit zarezerwowane dla repla
...
(define builtins '(number? boolean? procedure? pair? + - * / = < > pair fst snd not display number->string))
(define (op->builtin op)
  (match op
    ...
    ('eq?  (mk-unop 'quote eq?))
    ('display (mk-unop 'string display))
    ('number->string (mk-unop 'number number->string))))
...
(define (eval-env e ρ)
  (match e
    ...
    [(read-expr) (let ((input (read)))
                    (cond 
                    [(symbol? input)
                        (symbol->string input)]
                    [(number? input)
                        input]
                    [else (raise (type-error 'symbol input))]))]
    [(letrec-expr x e1 e2)
...
(eval (parse '(letrec (loop (lambda (input) 
                (if (and (number? input)
                         (= input 0))
                    (display "KONIEC")
                    (loop (read)))))
