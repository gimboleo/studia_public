...
(define (name->builtin op)
  (case op
    ...
    ['pair (builtin-binary (lambda (x y) (pair-val x y)))]
    ['fst (builtin pair-val-fst)]
    ['snd (builtin pair-val-snd)]))
 

(define builtins '(+ - * / = < and or pair fst snd))
          ...
          (let ([eql (typeeq-l eq)]
            ...
            (cond
              ...
              [(and (pair-type? eql)
                    (pair-type? eqr))
               (iter s (typeeqs-concat
                        (list (typeeqs-single (pair-type-t1 eql)
                                              (pair-type-t1 eqr))
                              (typeeqs-single (pair-type-t2 eql)
                                              (pair-type-t2 eqr))
                              eqs1)))]
              ; eliminacja
              [(var-type? eql)
