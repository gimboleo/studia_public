...
(define-type Expr
  ...
  (let-rec [id : Symbol] [e1 : Expr] [e2 : Expr]) ; <--- ZADANIE 3
    ...
    [(s-exp-list? q)
     (let ([ql (s-exp->list q)])
       (cond
         ...
         [(and (= (length ql) 3)      ; <---- ZADANIE 3
               (equal? (first ql) `let-rec))
          (let ([ll (s-exp->list (second ql))])
            (let-rec (s-exp->symbol (first ll))
                      (parse (second ll))
                      (parse (third ql))))]
...
(define (menv-add x v menv)
  (cons (pair x (box v)) menv))

(define (menv-update! x v menv)
  (type-case (MEnv 'a) menv
    [empty (error 'env-update! (string-append "Unknown identifier"
                                              (to-string x)))]
    [(cons p xs)
     (if (eq? (fst p) x)
         (set-box! (snd p) v)
...
(define (eval-menv e menv) ; <------------------------------------------
  (type-case Expr e
    [(const-num  n) (number-val  n)]
    [(const-bool b) (boolean-val b)]
    [(var-expr x) (menv-lookup x menv)]
    [(let-expr x e1 e2)
     (eval-menv e2 (menv-add x (eval-menv e1 menv) menv))]
    [(let-rec x e1 e2) ; <------ ZADANIE 3
     [let* ((menv (menv-add x (blackhole) menv))
            (val (eval-menv e1 menv)))
       (begin (menv-update! x val menv)
       (eval-menv e2 menv))]] 
    [(if-expr eb et ef)
     (if (boolean-val-v (eval-menv eb menv))
         (eval-menv et menv)
         (eval-menv ef menv))]
    [(fun x e) (closure x e menv)]
    [(app e1 e2)
     (let ([v1 (eval-menv e1 menv)]
           [v2 (eval-menv e2 menv)])
       (type-case Value v1
         [(builtin f) (f v2)]
         [(closure x e cenv) (eval-menv e (menv-add x v2 cenv))]
         [else (error 'eval-menv "Invalid applied value!")]))]))
    

(define (eval e) (eval-menv e start-menv)) ; <---------------------
...
(define (typecheck-env e env)
  (type-case Expr e
    ...
    [(let-expr x e1 e2)
     ...
     (let* ([t (gen-var)]
            [en (env-add x t env)]
            [r1 (typecheck-env e1 en)]
            [r2 (typecheck-env e2 en)])
       (typecheck-result (typecheck-result-tp r2)
                         (typeeqs-concat (list (typecheck-result-eqs r1)
                                         (typecheck-result-eqs r2)
                                         (typeeqs-single (typecheck-result-tp r1)
                                                t)))))]
    [(if-expr eb et ef)
...
(typecheck-and-eval (parse `(let-rec (fact (lambda (x) (if (= x 0) 1 (* x (fact (- x 1)))))) (fact 4)))) ; ZADANIE 3