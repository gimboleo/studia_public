...
(define (menv-lookup x menv)
  (type-case (MEnv 'a) menv
    [empty (error 'menv-lookup (string-append "Unknown identifier"
                                             (to-string x)))]
    [(cons p xs)
     (if (eq? (fst p) x)
         (unbox (snd p))
...
(define (menv-update! x v menv)
  (type-case (MEnv 'a) menv
    [empty (error 'menv-update! (string-append "Unknown identifier"
                                              (to-string x)))]
    [(cons p xs)
     (if (eq? (fst p) x)
         (set-box! (snd p) v)
...
(eval (parse `(let-rec (fact (lambda (x) (if (= x 0) 1 (* x (fact (- x 1))))))(fact 4)))) ; ZADANIE 3