...
(define-type Expr
  ...
  (quote-expr [q : Symbol]))  ;; <----- ZADANIE 1 typ składni abstrakcyjnej
       ...
       (cond
         ...
         [(and (= (length ql) 3)
          ...
          (make-funs (map s-exp->symbol (s-exp->list (second ql)))
                     (parse (third ql)))]
         [(and (= (length ql) 2) ;; <----- ZADANIE 1 
               (equal? (first ql) `quote))
          (quote-expr (s-exp->symbol (second ql)))]
         [else
...
(define-type Value
  ...
  (symbol [name : Symbol])  ;; <----- ZADANIE 1 typ wartości
...
(define (quote-op f)  ;; <----- ZADANIE 1 
  (lambda (x y)
    (boolean-val (f (symbol-name x) (symbol-name y)))))
...
(define (name->builtin op)
  (case op
    ...
    ['eq-symbol? (builtin-binary (quote-op eq?))]))  ;; <----- ZADANIE 1 środowisko startowe

(define builtins '(+ - * / = < and or eq-symbol?))  ;; <----- ZADANIE 1 
...
(define (eval-env e env)
  (type-case Expr e
    ...
    [(quote-expr q) (symbol q)]))  ;; <----- ZADANIE 1 ewaluator
...
(define-type Type
  ...
  (var-type [id : Symbol])
  (quote-type))  ;; <----- ZADANIE 1 typ typów
  ...
  (type-case Type t
    ...
    [(function-type t1 t2)
     (function-type (substitute s t1)
                    (substitute s t2))]
    [(quote-type) (quote-type)]))  ;; <----- ZADANIE 1 
...
(define (name->type op)
  (case op
    ...
    [(eq-symbol?) (function2-type (quote-type) (quote-type) (boolean-type))]))  ;; <----- ZAD. 1 środowisko startowe typów o typ operatora eq-symbol?,
...
(define (typecheck-env e env)
  (type-case Expr e
    ...
    [(quote-expr q)  ;; <----- ZADANIE 1 procedura typecheck-env
     (typecheck-result (quote-type) typeeqs-empty)]))
          ...
          (let ([eql (typeeq-l eq)]
            ...
            (cond
              ...
              [(and (boolean-type? eql)
                    (boolean-type? eqr))
               (iter s eqs1)]
              ; usunięcie  ;; <----- ZADANIE 1 algorytm unifikacji
              [(and (quote-type? eql)
                    (quote-type? eqr))
               (iter s eqs1)]
              ; dekompozycja
...
(typecheck-and-eval (parse `(quote x)))
(typecheck-and-eval (parse `(eq-symbol? (quote x) (quote y))))
(typecheck-and-eval (parse `(eq-symbol? (quote x) (quote x))))
(typecheck-and-eval (parse `(let [x-sym (quote x)] (let [y-sym (quote y)] (eq-symbol? x-sym y-sym)))))
; przykład: błąd typu
