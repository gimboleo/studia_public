  ...
  (match e
    ...
    [(binop op el er)
     (let* ((vl (eval-expr ϑ ρ el))
       ...
       ((op->proc op) vl vr))]
    [(call f es)
     (match (env-lookup f ϑ)                                                                               ;!
       [(not-found) (raise (unbound-variable f))]                                                          ;!
       [(fun id xs e)                                                                                      ;!
       (if (= (length xs) (length es))                                                                     ;!
           (let* [(zip-vars (map cons xs es))                                                              ;!
                  (new-ρ (foldr (lambda (a b) (env-add (car a) (eval-expr ϑ ρ (cdr a)) b)) ρ zip-vars))]   ;!
             (eval-expr ϑ new-ρ e))                                                                        ;!
           (raise (eval-error "Incorrect arity")))])]))                                                    ;!

(define (eval-prog p)
  (match p                                                                            ;!
    [(prog funs expr)                                                                 ;!
     (let ([env (foldr (lambda (a b) (env-add (fun-name a) a b)) env-empty funs)])    ;!
       (eval-expr env env-empty expr))]))                                             ;!