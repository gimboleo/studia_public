...
(define (u-operator? op) ;<-------------2
  (member op '(number? boolean? pair?)))
...
(define (u-op->proc op) ;<-------------5
  (match op
    ('number? (lambda (x) (if (number? x) 1 0)))
    ('boolean? (lambda (x) (if (and (number? x) (or (= x 0) (= x 1))) 1 0)))
    ('pair? (lambda (x) (if (pair? x) 1 0)))))
