...
(define (parse e)
  (cond
    ...
    [(and (list? e) (eq? (length e) 3) (eq? (first e) 'and)) ;<--------------4
     (if-expr (parse (second e))
              (parse (third e))
              (const false))]
    [(and (list? e) (eq? (length e) 3) (eq? (first e) 'or)) ;<--------------4
     (if-expr (parse (second e))
              (const true)
              (parse (third e)))]
    [(and (list? e)
...
;Ewentualnie mozna bylo po prostu dodac leniwe przypadki to ewaluatora i nie usuwac or i and z predykatu operator?
;
;[(binop 'and el er)
; (and (eval-env el ρ) (eval-env er ρ))]
;[(binop 'or el er)
; (or (eval-env el ρ) (eval-env er ρ))]
;
;W ten sposob nie korzystamy z lambdy ktora obliczala by wartosci gorliwie i mamy elegancko leniwie