...
(define curry (eval-env (parse '(lambda (f x y) (f (pair x y)))) env-empty)) ;<------------------ !!!
(define uncurry (eval-env (parse '(lambda (f p) (f (fst p) (snd p)))) env-empty)) ;<------------------ !!!
(define not-2 (eval-env (parse '(lambda (x) (if x false true))) env-empty)) ;<------------------ !!!

(define env-init (env-add (env-add (env-add env-empty 'curry curry) 'uncurry uncurry) 'not not-2)) ;<------------------ !!!

;; zakładamy że (expr? e), wtedy (value? (eval e)) (lub błąd wykonania)
(define (eval e)
  (eval-env e env-init)) ;<------------------ !!!
;; procedury wbudowane lądują w środowisku początkowym
;; Teraz srodowisko poczatkowe nie jest puste, wbudujemy w nie procedury
