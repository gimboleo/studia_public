...
; dodajemy konstrukcje c[a/d]+r jako lukier syntaktyczny
; a nie jako nową konstrukcję (zmieniamy tylko parse)
;
...
(define (cxxr-list? cs)  ; <------------------------------------- !!!
  (define (xxr-list? cs)  ; <------------------------------------ !!!
    (cond ((null? cs) false)  ; <-------------------------------- !!!
          ((eq? #\r (car cs)) (null? (cdr cs)))  ; <------------- !!!
          ((member (car cs) '(#\a #\d)) (xxr-list? (cdr cs))) ; <-!!!
          (else false)))    ; <---------------------------------- !!!
  (and (eq? #\c (car cs))   ; <---------------------------------- !!!
       (xxr-list? (cdr cs))))  ; <------------------------------- !!!

(define (desugar-xxr cs e)  ; <---------------------------------- !!!
  (if (equal? cs (list #\r)) e  ; <------------------------------ !!!
      (match (car cs)       ; <---------------------------------- !!!
        [#\a (fst (desugar-xxr (cdr cs) e))]  ; <---------------- !!! 
        [#\d (snd (desugar-xxr (cdr cs) e))])))  ; <------------- !!!
...
(define (parse e)
  (cond
   ...
   [(and (list? e)                      ; <---------------------------------- !!!
         (eq? (length e) 2))            ; <---------------------------------- !!!
    (let ((cxxr (string->list (symbol->string (first e))))  ; <-------------- !!!
          (psecond (parse (second e)))) ; <---------------------------------- !!!
      (cond  [(cxxr-list? cxxr)         ; <---------------------------------- !!!
              (desugar-xxr (cdr cxxr) psecond)]  ; <------------------------- !!!   
             [(eq? 'fst (first e))      ; <---------------------------------- !!!
              (fst psecond)]            ; <---------------------------------- !!!
             [(eq? 'snd (first e))      ; <---------------------------------- !!!
              (snd psecond)]))]         ; <---------------------------------- !!!
...
(eval (parse '(caar (pair (pair 1 2) (pair 3 4)))))
(eval (parse '(cdar (pair (pair 1 2) (pair 3 4)))))
(eval (parse '(cadr (pair (pair 1 2) (pair 3 4)))))
(eval (parse '(cddr (pair (pair 1 2) (pair 3 4)))))