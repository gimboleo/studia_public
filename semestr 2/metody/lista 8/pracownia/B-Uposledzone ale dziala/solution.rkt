...
(define (eval-env e ρ)
  (match e
    ...
    [(var-expr x) (match (env-lookup ρ x) ;<------------ zmiana
                    [(env-val v) v] ;<------------ zmiana
                    [(env-thunk e1 ρ1) (eval-env e1 ρ1)])] ;<------------ zmiana
    [(fun x e)    (clo x e ρ)]
    [(app e1 e2)
     (let ((v1 (eval-env e1 ρ)))  ;<------------ zmiana
       (match v1
         [(clo x e ρ1) (eval-env e (env-add-lazy ρ1 x e2 ρ))]  ;<------------ zmiana
    ...
    [(pair-expr el er) (cons (env-thunk el ρ) (env-thunk er ρ))] ;<------------ zmiana
    [(fst e)
     (let ((v (eval-env e ρ)))
       (if (pair? v)
           (eval-env (env-thunk-expr (car v)) (env-thunk-env (car v))) ;<------------ zmiana
           (raise (type-error 'pair v))))]
    [(snd e)
     (let ((v (eval-env e ρ)))
       (if (pair? v)
           (eval-env (env-thunk-expr (cdr v)) (env-thunk-env (cdr v))) ;<------------ zmiana
           (raise (type-error 'pair v))))]
    [(let-expr x e1 e2)
     (let ((v (eval-env e1 ρ)))
...
(check-equal? (eval (parse '((lambda (x) (+ 3 3)) (/ 5 0)))) 6)
(check-equal? (eval (parse '(let [if-fun (lambda (b t e) (if b t e))] (if-fun true 4 (/ 5 0))))) 4)
(check-equal? (eval (parse '(fst (pair (+ 2 2) (/ 5 0))))) 4)
(check-equal? (eval (parse '(snd (fst (pair (pair (/ 5 0) (* 2 2)) (/ 5 0)))))) 4)
(check-equal? (eval (parse '(let (p (pair (lambda (x y) (+ 20 y)) (/ 1 0))) ((fst (snd (fst (pair (pair 1 p) 50)))) (/ 1 0) 2)))) 22)
(check-equal? (eval (parse '(let (y 0) (let (p (pair y y)) (let (y 10) (fst p)))))) 0)