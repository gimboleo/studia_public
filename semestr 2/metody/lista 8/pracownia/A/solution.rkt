...
(define (op-bin? op)
  (member op '(+ - * / and or = < > apply))) ;<------------ zmiana
(define (op-un? op)
  (member op '(fst snd null? pair? list?))) ;<------------ zmiana
...
(define (op->proc op)
  (match op
    ...
    ('apply (lambda (x y) (if (list? y) (my-app x y) (raise (type-error 'list y))))) ;<------------ zmiana
...
(define (reserved? s)
  (or (op-bin? s)
      (op-un? s)
      (member s '(let if pair lambda null true false quit list)))) ;<------------ zmiana
...
(define (my-app f es) ;<------------ zmiana
  (match f ;<------------ zmiana
    [(clo x e ρ) (cond ;<------------ zmiana
                   [(null? es) f] ;<------------ zmiana
                   [(null? (cdr es)) (eval-env e (env-add ρ x (car es)))] ;<------------ zmiana
                   [else (my-app (eval-env e (env-add ρ x (car es))) (cdr es))])] ;<------------ zmiana
    [else (raise (type-error 'clo f))])) ;<------------ zmiana

(define (make-list es) ;<------------ zmiana
  (if (null? es) ;<------------ zmiana
      (const null) ;<------------ zmiana
      (pair-expr (car es) (make-list (cdr es))))) ;<------------ zmiana
...
(define (parse e)
  (cond
    ...
    [(and (list? e) ;<------------ zmiana
          (> (length e) 1) ;<------------ zmiana
          (eq? (first e) 'list)) ;<------------ zmiana
     (make-list (map parse (cdr e)))] ;<------------ zmiana
    [(and (list? e)
          (= 2 (length e))
          (op-un? (first e)))
...
(check-equal? (eval (parse '(apply (lambda (x y) (+ x y)) (list 1 2)))) 3)
(check-equal? (eval (parse '(apply (lambda (x y z) (+ x (+ y z))) (list 1 2 3)))) 6)
(check-equal? (eval (parse '(apply (lambda (x y) (lambda (z) (+ x (+ y z)))) (list 1 2 3)))) 6)
(check-equal? (eval (parse '(apply (lambda (x y) (+ x y)) (let (x 2) (list 1 x))))) 3)