...
(struct env-val (val)) ;<------------ zmiana
(struct env-thunk (expr env)) ;<------------ zmiana
(struct env (assoc))
...
(define (env-add ρ x v)
  (env (cons (cons x (env-val v)) (env-assoc ρ)))) ;<------------ zmiana
(define (env-add-lazy ρ x e ρ1) ;<------------ zmiana
  (env (cons (cons x (env-thunk e ρ1)) (env-assoc ρ)))) ;<------------ zmiana
...
(define (eval-env e ρ t) ;<------------ zmiana - W eval-env dochodzi wartosc t jako licznik fst i snd ktore szukaja par, w wiekszosci wiele nie zmieniaja, wiec nie zaznaczam
  (match e                                      ;dodania tej wartosci do kazdego wystapienia eval-env
    [(const n)    n]
    [(var-expr x) (match (env-lookup ρ x) ;<------------ zmiana
                    [(env-val v) v] ;<------------ zmiana
                    [(env-thunk e1 ρ1) (eval-env e1 ρ1 t)])] ;<------------ zmiana
    [(fun x e)    (clo x e ρ)]
    [(app e1 e2)
     (let ((v1 (eval-env e1 ρ t)))  ;<------------ zmiana
       (match v1
         [(clo x e ρ1) (eval-env e (env-add-lazy ρ1 x e2 ρ) t)]  ;<------------ zmiana
         [else        (raise (type-error 'procedure v1))]))]
    [(binop op el er)
     (let ((vl (eval-env el ρ t))
           (vr (eval-env er ρ t)))
       ((op->proc op) vl vr))]
    [(if-expr eb et ef)
     (let ((vb (eval-env eb ρ t)))
       (cond
        [(eq? vb true)  (eval-env et ρ t)]
        [(eq? vb false) (eval-env ef ρ t)]
        [else           (raise (type-error 'boolean vb))]))]
    [(pair-expr el er)
     (if (> t 0) ;<------------ zmiana
         (cons (env-thunk el ρ) (env-thunk er ρ)) ;<------------ zmiana
         (cons (eval-env el ρ t) (eval-env er ρ t)))] ;<------------ zmiana
    [(fst e)
     (let ((v (eval-env e ρ (+ t 1)))) ;<------------ zmiana
       (if (pair? v)
           (eval-env (env-thunk-expr (car v)) (env-thunk-env (car v)) t) ;<------------ zmiana
           (raise (type-error 'pair v))))]
    [(snd e)
     (let ((v (eval-env e ρ (+ t 1)))) ;<------------ zmiana
       (if (pair? v)
           (eval-env (env-thunk-expr (cdr v)) (env-thunk-env (cdr v)) t) ;<------------ zmiana
           (raise (type-error 'pair v))))]
    [(let-expr x e1 e2)
     (let ((v (eval-env e1 ρ t)))
       (eval-env e2 (env-add ρ x v) t))]))

;; zakładamy że (expr? e), wtedy (value? (eval e)) (lub błąd wykonania)
(define (eval e)
  (eval-env e env-empty 0)) ;<------------ zmiana
