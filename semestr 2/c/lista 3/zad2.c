...
int char_to_val(char c)
{
    ...
    else if (c >= 'A' && c <= 'H') return c - 'A' + 1;
    else if (c >= 'J' && c <= 'N') return c - 'J' + 1;
    else if (c == 'P') return 7;
    else if (c == 'R') return 9;
    else if (c >= 'S' && c <= 'Z') return c - 'S' + 2;
    else return 0;
}

char computeChecksum(Car *car)
{
    int weight[17] = {8, 7, 6, 5, 4, 3, 2, 10, 0, 9, 8, 7, 6, 5, 4, 3, 2};
    char chars[] = "0123456789X";
    int sum = 0;
    for (int i = 0; i < 17; i++) sum = sum + (char_to_val(car->number[i]) * weight[i]);
    sum = sum % 11;
    car->checksum = chars[sum];
    return chars[sum];
}

bool isChecksumValid(Car *car)
{
    computeChecksum(car);
    return (car->checksum == car->number[8]);
}
...
Car decodeVin(char vin[]) 
{
    ...
    year(pointer);
    printf(isChecksumValid(pointer) ? "true\n" : "false\n");
    printf("%c\n", samochod.checksum);
    printf("%s\n", samochod.number);
    printf("%s\n", samochod.region);
    printf("%s\n", samochod.country);
...
int main() 
{
    ...
    char autko4[] = "JH4KA3150JC014805";        //1988/2018 Acura (Japan)              //Brak Acura w bazie
    Car a = decodeVin(autko4);

    char autko5[] = "ZAMGJ45A480022578";        //2008/2038 Maserati  (Italy)          //Brak Włoch i Maserati w bazie
    Car b = decodeVin(autko5);                  //Podmieniłem kilka cyfr w numerze Vin
